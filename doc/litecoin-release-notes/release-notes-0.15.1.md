Ravecoin Core version *0.15.1* is now available from:

  <https://download.ravecoin.org/ravecoin-0.15.1/>
...
A number of changes to the way Ravecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 raves per kB (0.00001 RVE). This is to prepare for dropping the minimum transaction fee to 0.1 raves per kB (0.0001 RVE) in the next release.
